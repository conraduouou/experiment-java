...
public class Date
{
    ...
    // static method that returns how many days there are in passed value
    // Postcondition: max days in month num with respect to year is returned
    public static int daysinMonth(int num, int year)
    {
        if (num > 12)
            num = 12;
        else if (num < 1)
        ...
        switch (temp % 2)
        {
            case 1:
                return 31;
            case 0:
                if (temp == 2 && flag == false && isLeapYear(year))
                    return 29;
                else if (temp == 2 && flag == false)
                    return 28;
                else
